...
static void open_logs(void) {
  const char *index_path = getenv("BOOSTER_LUI_SSL_LOG_INDEX");
  const char *payload_path = getenv("BOOSTER_LUI_SSL_LOG_PAYLOAD");
...
static void load_flags(void) {
  const char *skip_verify = getenv("BOOSTER_LUI_SSL_SKIP_VERIFY");
